...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('message_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.messages.id', ondelete='CASCADE'), nullable=False, unique=True),
        sa.Column('company_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.companies.id'), nullable=False),
        sa.Column('user_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.users.id'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('id', UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('conversation_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.conversations.id', ondelete='CASCADE'), nullable=False),
        sa.Column('company_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.companies.id'), nullable=False),
        sa.Column('user_id', UUID(as_uuid=True), sa.ForeignKey('sred_ds.users.id'), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index('ix_feedback_alerts_company_id', table_name='feedback_alerts', schema='sred_ds')
    op.drop_index('ix_feedback_alerts_active', table_name='feedback_alerts', schema='sred_ds')
    op.drop_table('feedback_alerts', schema='sred_ds')

    op.drop_index('ix_feedback_aggregates_company_id', table_name='feedback_aggregates', schema='sred_ds')
    op.drop_index('ix_feedback_aggregates_period', table_name='feedback_aggregates', schema='sred_ds')
    op.drop_table('feedback_aggregates', schema='sred_ds')

    op.drop_index('ix_message_quality_scores_confidence', table_name='message_quality_scores', schema='sred_ds')
    op.drop_index('ix_message_quality_scores_company_id', table_name='message_quality_scores', schema='sred_ds')
    op.drop_table('message_quality_scores', schema='sred_ds')

    op.drop_index('ix_conversation_signals_company_id', table_name='conversation_signals', schema='sred_ds')
    op.drop_index('ix_conversation_signals_conversation_id', table_name='conversation_signals', schema='sred_ds')
    op.drop_table('conversation_signals', schema='sred_ds')

    op.drop_index('ix_message_feedback_details_rating', table_name='message_feedback_details', schema='sred_ds')
    op.drop_index('ix_message_feedback_details_company_id', table_name='message_feedback_details', schema='sred_ds')
    op.drop_table('message_feedback_details', schema='sred_ds')

    op.drop_column('messages', 'feedback_category', schema='sred_ds')
